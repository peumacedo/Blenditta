...
export function ReceivablesCard({ rows, resumo }: { rows: Array<{ id: string; cliente: string; vencimento: Date; valor: number; status: "PENDENTE" | "PAGO" | "RECEBIDO" | "VENCIDO"; dias: number }>; resumo: { vencido: number; aVencer: number; emAberto: number; maior: number } }) {
  return (
    <Card>
      ...
      <CardContent className="space-y-3">
        ...
        <div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Cliente/Origem</TableHead><TableHead>Vencimento</TableHead><TableHead className="text-right">Valor</TableHead><TableHead>Status</TableHead><TableHead>Dias</TableHead></TableRow></TableHeader><TableBody>{rows.map((item) => <TableRow key={item.id}><TableCell>{item.cliente}</TableCell><TableCell>{formatDate(item.vencimento)}</TableCell><TableCell className="text-right">{formatCurrency(item.valor)}</TableCell><TableCell>{situacaoContaLabel[item.status]}</TableCell><TableCell>{item.dias < 0 ? `${Math.abs(item.dias)} em atraso` : `${item.dias} para vencer`}</TableCell></TableRow>)}</TableBody></Table></div>
      </CardContent>
    </Card>
  );
}
